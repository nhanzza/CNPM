...
export default function EnhancedReportsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (hasPaidOrdersWithItems) {
        ...
        console.log('No order items found, showing empty product/customer charts')
        calculateChartsWithoutItems(allOrders)
      }
    } catch (error) {
  ...
  const calculateChartsWithoutItems = (ordersData: Order[]) => {
  ...
  const renderSimpleChart = () => {
    ...
    const highestValue = hasData ? Math.max(...data) : 0
    const averageValue = hasData ? totalValue / data.length : 0
          ...
          <div className="p-6 space-y-6">
            {!hasData ? (
              <div className="bg-gray-50 rounded-lg p-6 text-center text-sm text-gray-600">
                {t('Không có dữ liệu cho khoảng thời gian này.', 'No data available for this period.')}
              </div>
            ) : (
              <>
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          <div className="text-right w-28">
                            ...
                            <p className="text-xs text-gray-500">
                              {(totalValue > 0 ? (data[idx] / totalValue) * 100 : 0).toFixed(1)}%
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center border-l border-indigo-200">
                      ...
                      <p className="text-xl font-bold text-green-600 mt-1">
                        {showCustomChart === 'orders' ? highestValue.toLocaleString() : formatCurrency(highestValue)}
                      </p>
                    </div>
                    <div className="text-center border-l border-indigo-200">
            ...
            <div className="space-y-4">
              {chartData.topProducts.length === 0 ? (
                <div className="text-sm text-gray-500 italic">
                  {t('Chưa có dữ liệu sản phẩm bán chạy trong khoảng thời gian này.', 'No best-selling product data in this period.')}
                </div>
              ) : (
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="space-y-4">
              {chartData.topCustomers.length === 0 ? (
                <div className="text-sm text-gray-500 italic">
                  {t('Chưa có dữ liệu khách hàng VIP trong khoảng thời gian này.', 'No VIP customer data in this period.')}
                </div>
              ) : (
